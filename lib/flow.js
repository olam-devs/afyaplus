...
async function handleConfirm(to, lang) {
  ...
  // Try to create a Snippe payment session (mobile money + QR)
  let paymentUrl = null;
  if (process.env.SNIPPE_API_KEY && svc) {
    try {
      const snippeSession = await snippe.createPaymentSession({
        amount: svc.amount,
      ...
      paymentUrl = snippeSession.payment_link_url || snippeSession.checkout_url;
      console.log("Snippe session created:", { ref: snippeSession.reference, url: paymentUrl });
    } catch (err) {
      console.error("Snippe session error:", err.message);
    }
  }
