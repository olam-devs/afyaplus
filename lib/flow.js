...
async function sendServiceDetail(to, lang, serviceKey, bodyText, backId) {
  ...
  // Only add back button if it differs from menu button
  if (backId !== menuId) {
...
async function handleMessage(to, message) {
  ...
  if (session && (session.state === "awaiting_name" || session.state === "awaiting_phone")) {
    ...
    // Non-text (sticker/image/voice) during booking — remind user to type
  ...
  // Fallback for any unhandled message type (sticker/image/voice outside booking)
  await sendText(to, C.unrecognized);
}
