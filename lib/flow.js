...
async function handleConfirm(to, lang) {
  ...
  // Trigger Snippe USSD push directly to customer's phone
  ...
  if (process.env.SNIPPE_API_KEY && svc) {
    try {
      ...
      console.log("Snippe mobile payment initiated:", { ref: payment.reference, phone: session.phone });
    } catch (err) {
  ...
  sendBookingEmail({
    ...
    lang,
    paymentUrl: null,
    paymentStatus: "Pending (USSD sent)",
  ...
  if (paymentInitiated) {
    await sendButtons(to, t.successUssd(name, session.phone, serviceName, servicePrice), [
...
async function handleInteractive(to, id) {
  ...
  // Test payment (low-amount service for testing Snippe integration)
  if (key === "test_pay") {
    await startBooking(to, lang, "test_pay");
    return;
  }
