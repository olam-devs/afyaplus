...
const snippe = require("./snippe");
const { sendBookingEmail } = require("./email");

// Trigger words (case-insensitive)
const WELCOME_TRIGGERS = [
  // Short greetings
  "hi", "hello", "hey", "habari", "mambo", "niaje", "start", "anza", "karibu",
  "salaam", "jambo", "hujambo", "afya", "afya+", "ndewo", "shikamoo",
  "marahaba", "sasa", "vipi", "good morning", "good afternoon", "good evening",
  "howdy", "greetings", "yo", "hi, ! please let us know how we can help you.",
  // Website WhatsApp button messages (pre-filled text from profile pages)
  "hello afyaplus, i need healthcare services",
  "hello afyaplus, i need a gp consultation",
  "hello afyaplus, i need a specialist consultation",
  "hello afyaplus, i need a home doctor visit",
  "hello afyaplus, i need corporate health solutions",
  "hello afyaplus, i need pharmacy advisory",
  "hello afyaplus, i need help choosing a service",
  "hello afyaplus, i am interested in corporate health solutions",
];
...
async function handleBookingText(to, session, text) {
  ...
  if (session.state === "awaiting_name") {
    ...
    if (name.length < 2 || !/^[a-zA-Z\s\-'.]+$/.test(name)) {
      await sendText(to, lang === "sw"
        ? "Tafadhali andika jina sahihi (herufi tu, mfano: Amina Juma Hassan):"
        : "Please enter a valid name (letters only, e.g. Amina Juma Hassan):");
      return;
    }
    await setSession(to, {
      ...session, state: "awaiting_contact_phone", name,
      lastActivity: Date.now(), reminderCount: 0, lastReminder: 0,
    });
    await sendText(to, t.askContactPhone(name));

  } else if (session.state === "awaiting_contact_phone") {
    ...
    await setSession(to, {
      ...session, state: "awaiting_payment_phone", contactPhone: phone,
      lastActivity: Date.now(), reminderCount: 0, lastReminder: 0,
    });
    await sendText(to, t.askPaymentPhone);

  } else if (session.state === "awaiting_payment_phone") {
    const phone = text.trim().replace(/[\s\-()]/g, "");
    if (!/^\+?\d{9,15}$/.test(phone)) {
      await sendText(to, t.invalidPhone);
      return;
    ...
    const confirmText = t.confirm(session.name, session.contactPhone, phone, svc.name, svc.price);
    await setSession(to, {
      ...session, state: "awaiting_confirm", paymentPhone: phone,
      lastActivity: Date.now(), reminderCount: 0, lastReminder: 0,
    });
...
async function handleConfirm(to, lang) {
  ...
  if (!session || !session.name || !session.contactPhone || !session.paymentPhone) {
    await sendWelcome(to);
    return;
  }
  const t = C.booking[lang];
  ...
  // Trigger Snippe USSD push directly to customer's payment phone
  let paymentInitiated = false;
  if (process.env.SNIPPE_API_KEY && svc) {
    try {
      const nameParts = session.name.trim().split(/\s+/);
      const payment = await snippe.createMobilePayment({
        amount: svc.amount,
        customerPhone: session.paymentPhone,
        customerFirstName: nameParts[0] || "",
        customerLastName: nameParts.slice(1).join(" ") || "",
        metadata: {
          whatsapp_phone: to,
          customer_name: session.name,
          contact_phone: session.contactPhone,
          payment_phone: session.paymentPhone,
          service_key: session.serviceKey,
      ...
      console.log("Snippe mobile payment initiated:", { ref: payment.reference, phone: session.paymentPhone });
    } catch (err) {
      console.error("Snippe payment error:", err.message);
    }
  }
  ...
  sendBookingEmail({
    ...
    paymentPhone: session.paymentPhone,
  ...
  if (paymentInitiated) {
    await sendButtons(to, t.successUssd(name, session.contactPhone, session.paymentPhone, serviceName, servicePrice), [
      { id: `${lang}_menu`, title: labels.menu },
    ]);
  } else {
    await sendButtons(to, t.successFallback(name, session.contactPhone, session.paymentPhone, serviceName, servicePrice), [
      { id: `${lang}_menu`, title: labels.menu },
    ]);
  }
}
...
async function handleMessage(to, message) {
  ...
  if (session && session.state) {
    ...
    // --- AWAITING TEXT INPUT (name, contact phone, or payment phone) ---
    if (["awaiting_name", "awaiting_contact_phone", "awaiting_payment_phone"].includes(session.state)) {
